...
//		if (Integer.parseInt(r.get("code").toString()) != 0) return r;
        ...
        List<WmsTransferOrderPurEntity> wmsTransferOrderPurList = wmsTransferOrderPurService.queryLatest(query);

        if(wmsTransferOrderPurList!=null &&!wmsTransferOrderPurList.isEmpty()&& wmsTransferOrderPurList.size()>0){
            return R.ok().put("toNo",wmsTransferOrderPurList.get(0).getToNo()).put("toId",wmsTransferOrderPurList.get(0).getToPurId());
        }
